...
export default function PaymentMethodsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/payment-methods", { headers: { "x-actor-id": session.id } });
      const data = (await res.json()) as { payment_methods?: Row[]; error?: string };
      if (!res.ok) {
  ...
  async function toggleActive(r: Row, next: boolean) {
    if (!session) return;
    const res = await fetch(`/api/payment-methods/${r.id}`, {
      method: "PATCH",
    ...
    <div className="space-y-4 pb-8">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-slate-500">Defaults added when list is empty</p>
        </div>
        <button
          type="button"
      ...
      {!loading && filtered.length === 0 ? <p className="text-sm text-slate-500">No payment methods.</p> : null}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/40">
      ...
      <div className="mx-auto w-full max-w-[430px] rounded-t-2xl bg-white p-5 shadow-lg">
        <h2 className="text-lg font-semibold text-[#2563EB]">New payment method</h2>
