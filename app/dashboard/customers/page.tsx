...
export default function CustomersPage() {
  ...
  const canAccess = session?.role === "ceo" || session?.role === "ops";

  const load = useCallback(async () => {
    if (!session || !canAccess) return;
    setLoading(true);
    try {
      const res = await fetch("/api/customers", { headers: { "x-actor-id": session.id } });
      const data = (await res.json()) as { customers?: Row[]; error?: string };
      if (!res.ok) {
  ...
  async function removeRow(r: Row) {
    if (!session || !isCeo || r.entry_count > 0) return;
  ...
  if (!canAccess) {
    return (
      <div className="rounded-xl border border-red-200 bg-white p-6 text-center text-red-700 shadow-sm">Access denied</div>
    );
  }
    ...
    <div className="space-y-4 pb-8">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-slate-500">{isCeo ? "CEO: full control" : "Data operator: create only"}</p>
        </div>
        <button
          type="button"
        ...
        {filtered.map((r) => (
          <li key={r.id} className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-3 text-sm shadow-sm">
            ...
            {isCeo ? (
              ...
              <p className="text-xs text-slate-500">Contact CEO to deactivate or remove.</p>
            )}
          </li>
        ))}
      </ul>

      {!loading && filtered.length === 0 ? <p className="text-sm text-slate-500">No customers.</p> : null}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/40">
      ...
      <div className="mx-auto w-full max-w-[430px] rounded-t-2xl bg-white p-5 shadow-lg">
        <h2 className="text-lg font-semibold text-[#2563EB]">New customer</h2>
