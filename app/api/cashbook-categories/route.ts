...
export async function GET(request: Request) {
  ...
  let q = supabase.from("cashbook_categories").select("id, name, is_active, created_at");
  if (activeOnly) q = q.eq("is_active", true);
  const { data, error } = await q;
  if (error) return NextResponse.json({ error: "fetch_failed" }, { status: 500 });

  const rows = sortByNameCi(data ?? []);
  const { data: usage } = await supabase.from("cash_entries").select("category_id");
...
export async function POST(request: Request) {
  ...
  const supabase = createServiceClient();
  const { data: existing } = await supabase.from("cashbook_categories").select("id, name");
  ...
  return NextResponse.json({ category: { ...row, entry_count: 0 } });
}
